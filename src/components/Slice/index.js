...
const Slice = props =>{
  ...
  return pie.map((slice,index)=>{
    ...
    console.log("slice value is",slice,...arc.centroid(slice));
    return (<g key={index}>
      <path d ={arc(slice)} fill={sliceColor}/>
      <text dy=".35em" transform={translate(...arc.centroid(slice))} style={{"textAnchor":"middle","fill":"#fff","fontSize":"18px","fontWeight":"bold"}}>
